...
def get_model(model=None, m=False, pretrained=False, label_num=12):
    mdl = torch.nn.DataParallel(model(label_num=label_num)) if m else model(label_num=label_num)
    if not pretrained:
        return mdl
    else:
...
def valid(epoch, speechmodel, val_dataloader, loss_fn, words, CODER, best_accuracy, best_loss, model_path, writer):
    ...
    if accuracy > best_accuracy:
        ...
        torch.save(speechmodel.state_dict(), os.path.join(model_path,"model_best_acc_%s.pth" %(CODER)))
...
def train_model(log_path, model_path, data_path, noise_path, words, num_workers, target_dataset, model_class, unknown_ratio, preprocess_fun, is_1d, reshape_size, BATCH_SIZE, epochs, CODER, learning_rate=0.01,
               feature_cfg={},bagging_num=1, pretrained=None, pretraining=False, MGPU=False, warmup=False):
    """
    ...
    if training == 1:
        for e in range(epochs):
            ...
            best_accuracy, best_loss, epoch_loss = valid(e, speechmodel, valloader, loss_fn, words, CODER, best_accuracy, best_loss, model_path, writer)
    ...
    test_acc, test_loss = final_test(speechmodel, testloader, loss_fn, words)
